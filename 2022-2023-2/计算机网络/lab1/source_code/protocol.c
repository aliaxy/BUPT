...
unsigned int get_ms(void) {
  ...
  return (unsigned int)(epoch ? (tm.tv_sec - epoch) * 1000 + tm.tv_usec / 1000
                              : 0);
}
...
static void config(int argc, char **argv) {
  ...
  usage:
    ...
    printf("\nOptions : \n"
           ...
           "    -i, --ibib  : set station B layer 3 sender mode as "
           "IDLE-BUSY-IDLE-BUSY-...\n"
           "    -n, --nolog : do not create log file\n"
           "    -d, --debug=<0-7>: debug mask (bit0:event, bit1:frame, "
           "bit2:warning)\n"
  ...
  lprintf("Protocol.lib, version %s, jiangyanjun0718@bupt.edu.cn\n", VERSION,
          __DATE__);
  lprintf("Channel: %d bps, %d ms propagation delay, bit error rate ", CHAN_BPS,
          CHAN_DELAY);
  if (ber > 0.0)
    lprintf("%.1E\n", ber);
  else
...
static int sq_len(void) { return (sq_tail + SQ_SIZE - sq_head) % SQ_SIZE; }

int phl_sq_len(void) { return sq_len(); }
...
static void socket_recv(void) {
  ...
  if (ber != 0.0) {
    ...
    a = (int)((1.0 - pow(1.0 - ber, fact * blk->wptr)) * (RAND_MAX + 1.0) +
              0.5);
    if (rand() <= a) {
...
void stop_ack_timer(void) { timer[ACK_TIMER_ID] = 0; }
...
void enable_network_layer(void) { network_layer_active = 1; }

void disable_network_layer(void) { network_layer_active = 0; }
...
int recv_frame(unsigned char *buf, int size) {
  ...
  if (size < len) {
    sprintf(msg,
            "recv_frame(): %d-byte buffer is too small to save %d-byte "
            "received frame",
            size, len);
    ABORT(msg);
  }
  ...
  for (;;) {
    ...
    if (1) {
      ...
      if (t > mode_tick + 50 && time(0) > last_warn + 1) {
        lprintf("** WARNING: System too busy, sleep %d ms, but be awakened %d "
                "ms later\n",
                mode_tick, t);
        last_warn = time(0);
      }
