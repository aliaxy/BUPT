...
static int int64_str(char *s, int size, unsigned __int64 i, int base,
                     char upcase) {
  char *p;
  unsigned int j = 0;
...
static int output_string(const char *str, int size, int prefix_len, int width,
                         int precision, int flag, char pad,
                         char precision_pad) {
  const char *prefix;
  int len = 0;
...
static int output_integer(unsigned __int64 num, int opt_long, char type,
                          int width, int precision, int flag, int base,
                          char *prefix, char pad) {
  char buf[128], *s;
  int sz, prefix_len, n;
  bool is_negative;
...
static int output_double(double d, char type, int width, int precision,
                         int flag, int pad) {
...
static int output_memory_block(const unsigned char *ptr, int n, int width,
                               int precision, int flag, int pad) {
  ...
  next_option:
    ...
    case 's':
      ...
      if ((flag & F_DOT) && n > (signed)precision)
        n = precision;
      flag &= ~F_DOT;
      len += output_string(s, n, 0, width, 0, flag, ' ', ' ');
      break;
    ...
    print_num:
      ...
      len += output_integer(num, opt_long, ch, width, precision, flag, base,
                            prefix, pad);
      break;
    ...
    case 'e':
    case 'E':
      len += output_double(va_arg(arg_ptr, double), ch, width, precision, flag,
                           pad);
      break;
    ...
    case 'M':
      ...
      len += output_memory_block(ptr, va_arg(arg_ptr, int), width, precision,
                                 flag, pad);
      break;
